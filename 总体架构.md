...
│  │                                                                       │  │
│  │  # 列出实例                                                           │  │
│  │  curl http://192.168.x.x:8765/api/instances                           │  │
│  │                                                                       │  │
│  │  # 执行脚本                                                           │  │
│  │  curl -X POST http://192.168.x.x:8765/api/execute \                  │  │
...
# GET /api/config
{
  ...
  "curl_examples": {
    "list_instances": "curl http://192.168.1.100:8765/api/instances",
    "execute": "curl -X POST http://192.168.1.100:8765/api/execute -H 'Content-Type: application/json' -d '{\"instance_id\": \"INSTANCE_ID\", \"code\": \"print(len(db.functions))\"}'"
  }
}
```
...
```python
# skill 执行逻辑
HUB_URL = "http://192.168.1.100:8765"  # 从环境变量或配置读取
