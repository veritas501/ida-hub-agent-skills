      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-7 sm:px-6">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">
              在一个桌面视图中查看当前在线的 IDA 实例，并针对指定目标执行代码与排查连接状态。
            </p>
          </div>
          ...
          <div className="app-subcard flex flex-wrap items-center gap-4 px-4 py-3 lg:justify-end">
            <div>
              ...
              <p className="mt-1 text-lg font-semibold text-[var(--text)]">{instanceCount}</p>
            </div>
            <div className="hidden h-10 w-px bg-[var(--line)] sm:block" aria-hidden />
            <div>
              <p className="app-section-label">Refresh</p>
              <p className="mt-1 text-sm text-[var(--muted)]">Auto refresh every 5 seconds</p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          <section className="app-state-panel">
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">正在加载实例列表，请稍候...</p>
          </section>
        ) : null}
        ...
        {error ? (
          <section className="app-state-panel app-state-panel-error">
            ...
            <pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-words text-sm" style={{ fontFamily: "var(--font-mono)" }}>{String(error)}</pre>
