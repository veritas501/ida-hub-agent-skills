    ...
    return (
      <section className="app-state-panel app-state-panel-empty text-left">
        ...
        <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
          可以按下面顺序快速排查：确认 Hub 已启动、插件已执行
          Connect、并检查默认连接地址是否为
          <code
            className="app-inline-code ml-1"
        ...
        <ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-[var(--muted)]">
          ...
          <li>
            在 IDA 中通过 Edit -&gt; IDA Multi Chat -&gt; Connect 重新连接。
          </li>
          <li>
            若仍未出现实例，优先确认本机地址与 WebSocket 连接配置是否一致。
          </li>
        </ul>
      </section>
    );
