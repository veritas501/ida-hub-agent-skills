...
│  │  │  ├── /config        → SPA 路由    ├── /api/db-info              │  │  │
│  │  │  └── /_next/static  → 静态资源    └── WebSocket /ws             │  │  │
│  │  │                                                                  │  │  │
│  │  └─────────────────────────────────────────────────────────────────┘  │  │
│  │                                       ↑                               │  │
│  │                        InstanceRegistry                               │  │
...
> **说明**: `bitness`、`base_address`、`functions_count`、`strings_count`、`segments` 等详细信息通过 `/api/db-info/{instance_id}` 单独获取，不包含在实例列表中。
...
cd hub/web
npm install
npm run build          # 输出静态文件到 hub/web/out/
```
...
```
http://192.168.1.100:8765/           → 前端管理界面
http://192.168.1.100:8765/api/       → HTTP API
http://192.168.1.100:8765/docs       → API 文档
ws://192.168.1.100:8765/ws           → WebSocket（IDA 连接）
```
