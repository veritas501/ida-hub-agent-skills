# Skills 目录说明

本目录用于存放本仓库可复用的 Codex/Agent 技能定义。

## 目录结构

```text
skills/
├── README.md                 # 本说明文件
├── DEV.md                    # 开发指南
└── ida/                      # IDA Hub 逆向分析技能
    ├── SKILL.md              # 技能入口（默认执行策略、helper 模板、输出预算）
    └── references/           # 技能参考资料
        ├── USAGE.md          # 任务模板、脚本组织方式、排障
        ├── ANALYSIS_PATTERNS.md  # 高频逆向分析 playbook
        ├── API_REFERENCE.md  # ida-domain API 完整签名
        └── IDA.md            # IDA UI 交互（光标、选中、跳转）
```

## 技能设计约定

每个技能目录建议遵循以下最小结构：

- `SKILL.md`：必须存在，包含技能元信息与执行指引。
- `references/`：按需提供 API 参考、任务模板、排障文档。

`SKILL.md` 推荐包含：

- Frontmatter：
  - `name`：技能名（目录名建议一致）。
  - `description`：技能用途、触发场景与边界。
- 执行说明：
  - 默认执行方式（推荐 Python helper + 复合远程脚本）。
  - 输出预算约束（摘要优先，避免全量输出）。
  - 上下文约定（变量、运行环境、输入输出）。
  - 首次使用流程（必要前置条件）。
  - 错误处理与 fallback 路径。
  - 安全约束与风险提示。

## 当前技能

### `ida`

通过 HTTP API 与远程 IDA Hub Server 交互，执行逆向分析任务：

- 默认使用本地 Python helper 封装 Hub URL / instance_id / execute
- 业务逻辑写成复合远程脚本，在 IDA 端完成采集→过滤→聚合→摘要
- 默认只回传结构化摘要，避免全量输出
- `curl` 仅作为 fallback / 诊断路径

入口文件：`skills/ida/SKILL.md`

## 新增技能流程

1. 新建目录：`skills/<skill_name>/`
2. 编写 `SKILL.md`（先定义边界和默认执行策略，再补示例）
3. 按需补充 `references/*.md`
4. 在本 README 的"当前技能"补充条目
5. 自检：
   - 是否仅覆盖当前真实需求（YAGNI）
   - 是否避免重复描述（DRY）
   - 是否职责单一、边界清晰（SOLID）
   - 是否保持指令简洁可执行（KISS）

## 维护建议

- 优先小步迭代技能文档，避免一次性大而全重写。
- 新增任务模板时，遵循"采集→过滤→聚合→摘要"管道。
- 所有示例都需显式控制输出规模（Top N / count / json.dumps）。
- 参考文档只保留"高频且易错"的内容，降低维护成本。
